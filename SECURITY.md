...
MCP calls and document-parser workers have fixed active and queue limits. MCP
authentication has a 40-second cooperative deadline and a five-second lifecycle-lock
wait. Expired operations close owned browser/request resources and are fenced from
committing a late session update.
...
1. Run `npm ci`, `npm run typecheck`, `npm run build`, `npm test`, and `npm run security:audit`.
...
4. Verify the `CI and security` matrix and weekly offline verification pass.
